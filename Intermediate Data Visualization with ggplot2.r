...
ggplot(mtcars, aes(x = wt, y = mpg, color = fcyl)) +
  geom_point() +
  stat_smooth(method = "lm", se = FALSE) + stat_smooth(mapping = aes(group = 1), method = "lm", se = FALSE)


ggplot(mtcars, aes(x = wt, y = mpg)) +
  geom_point() +
  # Add 3 smooth LOESS stats, varying span & color
  stat_smooth(se = FALSE, color = "red", span = 0.9) + stat_smooth(se = FALSE, color = "green", span = 0.6) + stat_smooth(se = FALSE, color = "blue", span = 0.3)
...
ggplot(mtcars, aes(x = wt, y = mpg, color = fcyl)) +
  ...
  stat_smooth(se = FALSE, mapping = aes(color = "All")) +
  stat_smooth(method = "lm", se = FALSE)
...
ggplot(Vocab, aes(x = education, y = vocabulary, color = year_group)) +
  ...
  stat_smooth(method = "lm", mapping = aes(fill = year_group), size = 2)
...
scale_y_continuous(alist(sec_axis = secondary_y_axis)) +
  labs(x = "Date (1973)", y = "Fahrenheit")
...
ggplot(mtcars, aes(x = wt, y = mpg)) + 
  ...
  facet_grid(rows = vars(fvs, fam), cols = vars(gear), margins = c("fam"))
...
ggplot(mtcars_by_cyl, aes(cyl, mean_wt)) +
  ...
  geom_col(fill = "skyblue", mapping = aes(width = prop))

  # Add an errorbar layer
  + geom_errorbar(
    # ... at mean weight plus or minus 1 std dev
    mapping = aes(ymin = mean_wt - sd_wt, ymax = mean_wt + sd_wt),
    # with width 0.1
    width = 0.1
  )
...
# Change type
TG$dose <- as.numeric(x = as.character(TG$dose))
